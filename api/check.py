...
def check_single_cookie(cookie, cookie_id=0):
    ...
    # Step 1: Get CSRF token (required for Roblox API)
    try:
        csrf_url = "https://auth.roblox.com/v2/logout"
        csrf_response = requests.post(csrf_url, headers=headers, timeout=8, verify=False, allow_redirects=True)
    ...
    try:
        ...
        response = requests.get(auth_url, headers=headers, timeout=12, verify=False, allow_redirects=True)
            ...
            try:
                ...
                if user_id != 'Unknown':
                    try:
                        ...
                        profile_resp = requests.get(profile_url, headers=headers, timeout=8, verify=False)
                        ...
                        avatar_resp = requests.get(avatar_url, headers=headers, timeout=6, verify=False)
                        ...
                        premium_resp = requests.get(premium_url, headers=headers, timeout=8, verify=False)
                        ...
                        economy_resp = requests.get(economy_url, headers=headers, timeout=8, verify=False)
                        if economy_resp.status_code == 200:
                            result['robux'] = economy_resp.json().get('robux', 0)

                        # Get friends count
                        friends_url = f"https://friends.roblox.com/v1/users/{user_id}/friends/count"
                        friends_resp = requests.get(friends_url, headers=headers, timeout=8, verify=False)
        ...
        result['status'] = 'error'
        result['error'] = 'Connection error - tidak dapat terhubung ke Roblox'
